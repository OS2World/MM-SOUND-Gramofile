...
read_from_keyboard ()
{
  ...
  signed int left;		/* 'standard int' for %d */
  signed int right;

  printf ("Please enter left & right sample: ");

  scanf ("%d %d", &(left), &(right));

  inputsample.left = left;
  inputsample.right = right;
...
void
param_screen (parampointer_t parampointer, int filtertype)
{
  switch (filtertype)
    {
    case 0:			/* nothing needed */
    ...
    default:
      printf ("Error (praram_screen): wrong filter");
      exit (2);
      break;
    }
